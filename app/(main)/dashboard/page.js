...
function LucyBriefing({ language }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/lucy?language=${language}`)
      ...
      .catch(() => { setBriefing("Good morning! Let's make today productive. 💙"); setLoading(false); });
  }, [language]);
        ...
        <div style={{ flex: 1 }}>
          ...
          {loading ? (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 13, color: "#94a3b8" }}>{language === "kh" ? "Lucy កំពុងរៀបចំ..." : "Lucy is preparing your briefing..."}</span>
            </div>
          ) : (
            <p style={{ fontSize: 13, color: "#e2e8f0", lineHeight: 1.7, margin: 0, whiteSpace: "pre-wrap" }}>{briefing}</p>
          )}
          <Link href="/lucy" style={{ display: "inline-flex", alignItems: "center", gap: 6, marginTop: 10, fontSize: 12, color: "#4F8CFF", textDecoration: "none", fontWeight: 600 }}>
...
function SurveyBanner() {
  ...
  useEffect(() => {
    fetch("/api/survey").then(r => r.json()).then(d => {
      if (Array.isArray(d)) {
        setHasSurvey(d.some(s => !s.alreadySubmitted));
      }
    }).catch(() => {});
  }, []);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          <p style={{ fontSize: 14, fontWeight: 700, color: "white", margin: 0 }}>You have a survey to fill!</p>
          <p style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", margin: 0 }}>Share your feedback to help improve the system</p>
        </div>
      </div>
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto relative">
        ...
        <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
          <div className="animate-float absolute top-20 right-20 w-32 h-32 rounded-full opacity-5" style={{ background: "linear-gradient(135deg, #4F8CFF, #667eea)" }} />
          <div className="animate-float-delayed absolute top-40 left-10 w-20 h-20 rounded-full opacity-5" style={{ background: "linear-gradient(135deg, #a8edea, #4F8CFF)" }} />
        </div>
        ...
        <div className="relative z-10">
          ...
          {/* Due soon alert */}
          {dueSoon.length > 0 && (
            <div className="mb-4 p-4 rounded-2xl border border-orange-200 flex items-center gap-3" style={{ background: "linear-gradient(135deg, #fff7ed, #ffedd5)" }}>
              <span className="text-2xl">⚠️</span>
              <div>
                <p className="font-semibold text-orange-700 text-sm">Due within 24 hours!</p>
                <p className="text-orange-600 text-xs mt-0.5">{dueSoon.map(t => t.title).join(", ")} — don't forget!</p>
              </div>
            </div>
          )}

          {/* Quote */}
          <div className="mb-6 p-4 rounded-2xl text-white text-sm italic text-center" style={{ background: "linear-gradient(135deg, #4F8CFF 0%, #667eea 100%)" }}>
            💡 "{quote}"
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            {[
              ...
              { label: "Completed",   value: completed, color: "text-green-500", bg: "from-green-50 to-white" },
              { label: "Overdue",     value: overdue,   color: "text-red-500",   bg: "from-red-50 to-white" },
            ].map(stat => (
              <div key={stat.label} className={`bg-gradient-to-br ${stat.bg} rounded-2xl shadow-sm p-5 text-center`}>
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-6">
            ...
            <div className="md:col-span-2 bg-white rounded-2xl shadow-sm p-6">
              ...
              {upcoming.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <Link href="/lucy" className="text-sm text-[#4F8CFF] mt-2 inline-block hover:underline">Tell Lucy about your tasks →</Link>
                </div>
              ) : (
                    ...
                    return (
                      <div key={task.id} className="flex items-center justify-between p-4 bg-[#F0F4FF] rounded-xl">
                        <div className="flex items-center gap-3">
                          <div className={`w-2 h-8 rounded-full ${task.priority === "high" ? "bg-red-400" : task.priority === "medium" ? "bg-yellow-400" : "bg-green-400"}`} />
                          <div>
                            <p className="font-medium text-sm text-slate-800">{task.title}</p>
                            <p className="text-xs text-slate-500">{task.subject}</p>
                          </div>
                        </div>
                        <span className={`text-xs px-3 py-1 rounded-full font-medium ${daysLeft <= 2 ? "bg-red-100 text-red-600" : "bg-[#EBF1FF] text-[#4F8CFF]"}`}>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6">
              ...
              <div className="flex flex-col items-center justify-center py-2">
                <div className="relative w-32 h-32">
                  <svg className="w-32 h-32 -rotate-90" viewBox="0 0 120 120">
                    ...
                    <circle cx="60" cy="60" r="50" fill="none" stroke="url(#grad1)" strokeWidth="12"
              ...
              <div className="mt-4 space-y-2">
                <Link href="/lucy" className="flex items-center gap-2 p-3 bg-[#F0F4FF] rounded-xl hover:bg-[#EBF1FF] transition-all">
                  <span>🎓</span><span className="text-sm font-medium text-[#4F8CFF]">Chat with Lucy</span>
                </Link>
                <Link href="/planner" className="flex items-center gap-2 p-3 bg-[#F0F4FF] rounded-xl hover:bg-[#EBF1FF] transition-all">
                  <span>🤖</span><span className="text-sm font-medium text-[#4F8CFF]">Generate Study Plan</span>
                </Link>
                <Link href="/progress" className="flex items-center gap-2 p-3 bg-[#F0F4FF] rounded-xl hover:bg-[#EBF1FF] transition-all">
                  <span>📊</span><span className="text-sm font-medium text-[#4F8CFF]">View Full Progress</span>
                </Link>
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-4 mb-6">
            <div className="bg-white rounded-2xl shadow-sm p-5">
              <div className="flex items-center gap-3 mb-2"><span className="text-2xl">🎯</span><p className="text-sm text-slate-500">Completion Rate</p></div>
              <h3 className="text-2xl font-bold text-[#4F8CFF]">{percent}%</h3>
              <p className="text-xs text-slate-400 mt-1">of all tasks done</p>
            </div>
            <div className="bg-white rounded-2xl shadow-sm p-5">
              <div className="flex items-center gap-3 mb-2"><span className="text-2xl">📚</span><p className="text-sm text-slate-500">High Priority</p></div>
              <h3 className="text-2xl font-bold text-red-500">{tasks.filter(t => t.priority === "high" && t.status === "pending").length}</h3>
              <p className="text-xs text-slate-400 mt-1">tasks need attention</p>
            </div>
            <div className="bg-white rounded-2xl shadow-sm p-5">
              <div className="flex items-center gap-3 mb-2"><span className="text-2xl">✅</span><p className="text-sm text-slate-500">Done This Week</p></div>
              <h3 className="text-2xl font-bold text-green-500">
                {tasks.filter(t => { const d = new Date(t.createdAt); const w = new Date(); w.setDate(w.getDate()-7); return t.status === "completed" && d >= w; }).length}
              </h3>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-slate-400">{today.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}</span>
            </div>
            {todayTasks.length === 0 ? (
              <div className="text-center py-6">
                <p className="text-3xl mb-2">🎉</p>
                <p className="text-slate-400 text-sm">No tasks due today!</p>
                <Link href="/lucy" className="text-sm text-[#4F8CFF] mt-2 inline-block hover:underline">Ask Lucy what to study today →</Link>
              </div>
            ) : (
              <div className="space-y-3">
                {todayTasks.map(task => (
                  <div key={task.id} className="flex items-center justify-between p-4 bg-[#F0F4FF] rounded-xl">
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-8 rounded-full ${task.priority === "high" ? "bg-red-400" : task.priority === "medium" ? "bg-yellow-400" : "bg-green-400"}`} />
                      <div>
                        <p className={`font-medium text-sm ${task.status === "completed" ? "line-through text-slate-400" : "text-slate-800"}`}>{task.title}</p>
                        <p className="text-xs text-slate-500">{task.subject}</p>
                      </div>
                    </div>
                    <span className={`text-xs px-3 py-1 rounded-full font-medium ${task.status === "completed" ? "bg-green-100 text-green-600" : task.priority === "high" ? "bg-red-100 text-red-600" : task.priority === "medium" ? "bg-yellow-100 text-yellow-600" : "bg-green-100 text-green-600"}`}>
