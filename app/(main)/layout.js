...
export default function MainLayout({ children }) {
  ...
  return (
    <div className={`flex min-h-screen ${dark ? "bg-slate-900" : "bg-[#F0F4FF]"}`}>
      ...
      {/* ── Sidebar ── */}
      <aside
        className={`w-64 flex flex-col fixed h-full z-30 shadow-sm transition-transform duration-300 ${
              ...
              >
                ...
                {isLucy && !active && (
                  <span
                    ...
                    style={{ background: "linear-gradient(135deg,#4F8CFF,#667eea)", color: "white", fontSize: 10 }}
                  >
                    AI
                  </span>
                )}
      ...
      <div className="flex-1 md:ml-64 flex flex-col">
        ...
        >
          ...
          <button
            onClick={toggleLanguage}
            className={`p-2 rounded-xl text-lg ${dark ? "text-white" : "text-slate-700"}`}
          >
            {language === "en" ? "🇺🇸" : "🇰🇭"}
          </button>
        </div>
